    ...
    beforeEach((done) => {
      plugin.register({ batchInterval: 25, serverUrl: 'http://graphql' }
        , {}, () => {
          ...
          client.query(`{ metro(metroId: 4) { name } }`, { test: 1 }, { 'accept-language': 'en-US' });
          done();
        });
    });

    it('should call apollo client with merged headers', () => {
      const expectedResult = { query: sinon.match.any, variables: { test: 1, __headers: { 'accept-language': 'en-US' } } };
